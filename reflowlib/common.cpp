...
vector<cv::Rect> merge_overlapping_slices(vector<cv::Rect> rectangles)
{
    ...
    for (; first_slice != rectangles.end(); ++first_slice)
    {
        ...
        for (;
             second_slice != rectangles.end(); ++second_slice)
        {
            // Set the width of the first slice to be equivalent to the
            // rightmost point of the two rectangles.
            
            if (
                (first_slice->x <= second_slice->x + second_slice->width && first_slice->x+first_slice->width >= second_slice->x &&
                 first_slice->y >= second_slice->y + second_slice->height && first_slice->y + first_slice->height <= second_slice->y)
                ) {
                
                first_slice->width = std::max(
                                              first_slice->x + first_slice->width
                                              , second_slice->x + second_slice->width)
                - std::min(first_slice->x,second_slice->x);
                
                first_slice->height = std::max(
                                               first_slice->y + first_slice->height
                                               , second_slice->y + second_slice->height)
                - std::min(first_slice->y,second_slice->y);
                
                first_slice->x = std::min(first_slice->x, second_slice->x);
                first_slice->y = std::min(first_slice->y, second_slice->y);
                
                //std::cout << "right point = " <<  first_slice->x + first_slice->width  << std::endl;
                
                
                // Mark the second slice as having been merged.
                second_slice->height = 0;
                second_slice->width = 0;
                second_slice->x = 0;
                second_slice->y = 0;
